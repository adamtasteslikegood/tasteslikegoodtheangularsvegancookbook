...
- **gbrain and the Backend submodule** — if gbrain is configured on your machine, `Backend/` is indexed as a *separate, non-federated* source (`gstack-code-backend`), so `code-def`/`code-refs`/`search`/`query` against Backend Python need an explicit `--source gstack-code-backend` or they silently miss. Never run a bare `/sync-gbrain` from inside `Backend/`: it has no `.gbrain-source` pin, so the code stage registers the cwd as a *new* federated source and re-indexes the whole repo alongside the existing one. The nested-path guard does not catch it (the real source lives in gbrain's managed clone dir, so there is no path overlap). Verified with `--dry-run` 2026-07-24. Re-sync with `gbrain sync --source gstack-code-backend --strategy code`.
