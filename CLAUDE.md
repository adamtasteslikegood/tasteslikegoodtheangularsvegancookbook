...
- **OAuth scope removal is permanent for already-consented accounts.** Once a Google account has granted a scope to the cookbook, Google retains the grant and bundles it back into future token responses even after we stop requesting it. `oauthlib`'s `validate_token_parameters` then raises on the scope-set mismatch and `/api/auth/login/callback` returns HTTP 500 with `{"error":"Authentication failed"}`. Two-part mitigation lives in Backend (`397ba90` / PR #128): (1) `/api/auth/login` does NOT pass `include_granted_scopes=true` — the cookbook always requests its full scope set up front, and (2) `OAUTHLIB_RELAX_TOKEN_SCOPE=1` is set at module import as defense-in-depth. If you ever change the OAuth scope set in `Backend/auth.py`, treat the change as permanent — there is no clean rollback path. See `specs/planning_notes.md` "Post-Launch Findings" for the v0.2.5 incident.
- **Schema-changing PRs are not safe to ship until the migrate Job is verified.** v0.2.0/v0.2.1 went out without `flask db upgrade` ever running against Cloud SQL and 500'd in production. v0.2.2/v0.2.3 closed this with the `flask-backend-migrate` Cloud Run Job (see "Database migrations" above). Any PR adding or altering an Alembic migration must keep the Job green — a failing migration aborts the build and the old Flask revision keeps serving traffic.
