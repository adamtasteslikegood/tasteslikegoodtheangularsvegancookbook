...
#### No automation writes the retro — YOU do, as the last step of every sprint close-out

This file previously claimed these pages were "generated by a Jira/Atlassian automation from a
custom template." **That is false, and it was verified false on 2026-08-08 (KAN-216).** Sprint 5's
epic (RCP-65) was closed and then Jira sprint 46 was closed; no page appeared after either trigger.
Sprint 4's retro only _looked_ automated because it was written by an agent session minutes after
that sprint closed — every version of page `51019869` is authored by Adam's own atlassian account,
not by an app user, and the page body says so: _"Compiled by Claude on Adam's behalf."_ The "Sprint
Close Retrospective Template Content" page (`50495489`) is a **manual** template — its own first
line tells you to paste it into the space-template editor by hand.

Believing the automation existed would have stalled Sprint 6 chartering forever, waiting on a
trigger that never fires. Treat this as the standing example of the project's most expensive
failure mode: **a documented mechanism nobody ever watched actually fire** (cf. the Alembic
head-check, and `check_sprint_lane.sh` before it was wired into `gate.needs`).

**So: closing a sprint is not done until the retro page exists.** As the final close-out step,
create it yourself — read the template body from page `50495489`, fill it from the close-out plus
the board data, and `POST` it as a child of parent `50298881`:

```bash
set -a && source .env && set +a   # ATLASSIAN_EMAIL + ATLASSIAN_API_TOKEN
curl -u "$ATLASSIAN_EMAIL:$ATLASSIAN_API_TOKEN" -X POST \
  https://tasteslikegood.atlassian.net/wiki/rest/api/content \
  -H 'Content-Type: application/json' -d '{"type":"page",
  "title":"Sprint <N> Retrospective — <YYYY-MM-DD>","space":{"key":"TLG"},
  "ancestors":[{"id":"50298881"}],"body":{"storage":{"value":"<html>","representation":"storage"}}}'
```

Title format `Sprint <N> Retrospective — <YYYY-MM-DD>` — the "read the retro first" search above
matches on it. The **"Actions for Next Sprint" table is the point**: it is the one artifact that
exists nowhere in the repo, so a retro written without it is not a retro. Embed the gate evidence
in the footer while you still have it; after the close-out lands, that context is gone.

Do not build a Jira automation for this instead. The agent already does the close-out work and
already holds the gate evidence; an automation would be a second mechanism to keep in sync, firing
on whichever board happens to hold the sprint object, with no access to the evidence.
