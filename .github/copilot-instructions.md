...
- The Gemini client is lazily instantiated (`getClient()`) — app startup does not fail if `GEMINI_API_KEY` is absent.
...
| Error | Cause | Fix |
|---|---|---|
| `Missing API key` on first recipe/image request | `GEMINI_API_KEY` not set (Gemini client initialises lazily in `getClient()`) | Export env vars in shell (`export $(grep -v '^#' .env.local | xargs)`), then retry the request |
