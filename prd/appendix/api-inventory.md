...
| Method | Path                          | Auth                                          | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------ | ----------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | `/api/generate`               | guest OK                                      | Body `{prompt (10–500 chars required), model (defaults to gemini-3.7-flash, pinned via `GEMINI_DEFAULT_MODEL`in`cloudbuild.yaml`)}`. Creates a pending recipe row (`status="generating"`, name "Generating..."), publishes to Pub/Sub topic `recipe-generation`, returns **202** `{recipe_id, status: "generating"}`. 400 on prompt validation; 500 on DB/publish failure (status set to `error`). Express rate-limits to 20/hr/IP. |
...
| POST   | `/api/worker/image`  | Pub/Sub OIDC                                                                                       | Generates the food photo with `gemini-3-pro-image` (Nano Banana Pro), pinned via `GEMINI_IMAGE_MODEL` in `cloudbuild.yaml`; the prompt comes from name + image_keywords ("professional food photography… overhead shot"). GCS-first storage (`gs://$GCS_BUCKET_NAME/images/<id>.png`), base64-in-DB fallback when no bucket. Sets `ai_image_url=/api/recipes/<id>/image`. No retry; failures recorded in `ai_metadata.image_generation`. |
