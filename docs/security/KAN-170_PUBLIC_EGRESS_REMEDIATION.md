...
- **Recurrence detection**, closed 2026-07-28 (KAN-173). `kan170_verify.sh` no longer merely prints: it accumulates failures and **exits 1** on drift, and it now asserts `express-frontend`'s ingress directly, since ingress is that service's only guard. `.github/workflows/security-posture-check.yml` runs it daily against production.
  ...
  **The scheduled half was still dead on arrival, and was fixed on 2026-07-28.** It authenticated via `secrets.GCP_WORKLOAD_IDENTITY_PROVIDER` / `GCP_SERVICE_ACCOUNT`; neither secret existed, and the project had no GitHub Actions WIF provider at all — its only pool was GKE's `svc.id.goog`, which does not support providers. Every daily run would have failed at the auth step: red, but silent about posture. The same pattern one layer down — wired to a trigger, able to fail, and still unable to answer the question. It now federates through a dedicated `github-actions` pool whose provider is attribute-conditioned to this repository, assuming `posture-check@` (`run.viewer`, `pubsub.viewer`, `compute.networkViewer` — read-only, cannot deploy). Its secrets are deliberately named `GCP_POSTURE_*`: `gc-build-deploy.yml` reads a non-empty `GCP_SERVICE_ACCOUNT` as "deployment is configured", so reusing that name would have told the deploy path it was wired up while holding an identity that cannot deploy.

  **What the check asserts about `flask-backend` is "at least one guard holds", not "both".** Flask has two possible guards: invoker IAM enforcement (Path A, shipped) and closed ingress (Path B, never executed — KAN-176). Production runs Path A only: `ingress=all`, IAM enforced, anonymous `GET /` → 403. Demanding both would fail every run against the posture this document records as sufficient, and a permanently-red job is an ignored one — the drift it exists to catch would hide in its own noise. Both guards gone at once is the KAN-170 exposure exactly, and that is what fails, within 24h. `express-frontend`'s ingress assertion stays strict because ingress is its only guard. The first real production run (2026-07-28) exited 1 on the single-guard finding, which is how KAN-176 was opened — the control found it, not a human.
