...
| 2   | **Identity principle** | **Canonical URL + recipe NAME are the stable identity; recipe content may version-bump in place.** `-N` suffix slugs are the **by-design** output of the normalize-slug collision logic — legitimate duplicates exist (same name by one author, or across users) and the suffix disambiguates them correctly. The defect is **unintentional same-author siblings from regeneration** (regenerating an existing recipe should offer version-bump, not silently mint a new row); the suffix was merely a safe *filter heuristic* for this round's test artifacts. Feeds #3147 (renames must not casually mint new slugs).                                                            |
