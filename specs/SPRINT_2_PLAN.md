...
| C3  | **Prod content hygiene** — delete `zzz-racetest-*` + duplicate `Dooypkiitts` cookbooks, run `unpublish_slugs.py`, fix/unpublish the 2 remaining imageless recipes                                                  | ux-backlog #3 + #4       | zero test-data rows in prod listing AND zero published recipes with imageless heroes                                |
...
**Recommended commit (pending grill):** C1 + C2 + C3 — C1 is owner-directed and time-sensitive
...
2. **Adam confirms deletion list** for prod hygiene — blocks C3 (destructive, human-gated).
