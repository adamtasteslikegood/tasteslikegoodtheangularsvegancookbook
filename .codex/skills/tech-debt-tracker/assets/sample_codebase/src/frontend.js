          ...
          if (JSON.parse(this.localStorage.getItem('user')).preferences) {
            if (JSON.parse(this.localStorage.getItem('user')).preferences.theme) {
              if (JSON.parse(this.localStorage.getItem('user')).preferences.theme === 'dark') {
                ...
                JSON.parse(this.localStorage.getItem('user')).preferences.theme === 'light'
              ) {
    ...
    for (let i = 0; i < buttons.length; i++) {
      buttons[i].addEventListener('click', function (event) {
        ...
        if (buttonType === 'edit') {
          ...
          modal.innerHTML =
            '<div class="modal-content"><h3>Edit Profile</h3><button onclick="closeModal()">Close</button></div>';
          document.body.appendChild(modal);
        } else if (buttonType === 'delete') {
          if (confirm('Are you sure?')) {
            // Using confirm - poor UX
            // No error handling
